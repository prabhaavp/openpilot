...
# Widescreen Crop Resolution [W x H = 352 x 256]
MODEL_INPUT_W = 352
MODEL_INPUT_H = 256

# Default Dual-Threshold Hysteresis Rules
HYSTERESIS_ON = 0.99   # Requires >=99% confidence to trigger ON
HYSTERESIS_OFF = 0.95  # Stays ON until confidence drops below 95%
...
class VASMInference:
  ...
  def load(self) -> bool:
    ...
    try:
      ...
      self.last_error = f"Failed to load classification model: {e}"
      self._valid = False
    return self._valid
  ...
  def _run_inference(self, raw_image, height, side) -> float:
    ...
    # Convert cropped area directly from YUV NV12 to RGB
    ...
    # Preprocess -> NCHW Float32 [0.0 - 1.0] (W=352, H=256)
    ...
    # Classification Softmax Probability Extraction for Class 1 (1_car)
