...
class ModelRenderer(Widget):
  ...
  def _draw_path(self, sm):
    ...
    min_speed = self._params.get_int("MinPathSpeed", default=0)
    if min_speed > 0:
      current_speed_mph = ui_state.sm["carState"].vEgo * CV.MS_TO_MPH
  ...
  def _draw_lead_indicator(self):
    ...
    lead_smoothing = self._params.get_float("LeadSmoothingFactor", default=0.0) / 100.0
    for i, lead in enumerate(self._lead_vehicles):
      ...
      if lead_smoothing > 0.0:
        ...
        self._prev_lead_alphas[i] = lead.fill_alpha
        smoothed_alpha = lead.fill_alpha
      rl.draw_triangle_fan(lead.glow, len(lead.glow), rl.Color(218, 202, 37, 255))
      rl.draw_triangle_fan(lead.chevron, len(lead.chevron), with_alpha(lead_color, smoothed_alpha))
