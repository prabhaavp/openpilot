...
# curved-rectangle variant used on the mici display.
...
void main() {
  ...
  // Darken near the rounded border and let a faint rim blend into the UI.
...
class PipSideCamera(Widget):
  ...
  def _pick_side(self, sides: list[str]) -> str | None:
    ...
    Only a rising edge (inactive -> active) refreshes the timestamp, so a side
    that stays on doesn't re-beat one that just triggered.
    """
    now = time.monotonic()
    active = set(sides)
